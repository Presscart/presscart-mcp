---
name: commit
description: Use whenever the user wants to commit changes — phrases like "$commit", legacy "/commit", "commit this", "commit these changes", "make a commit", "stage and commit", "save this to git", "wrap this up", "write a commit message", or any time they want to checkpoint work in git history. Inspects working tree state, groups related files into one or more Conventional Commits with itemized bodies explaining the *why*, then stages and commits immediately — no preview or approval gate. Use this skill even when the user gives a vague directive like "commit it" without specifying scope or message — that's exactly when the skill earns its keep.
---

<!-- Mirrored skill: a Claude Code twin of this file ships in the kane-claude bundle (.claude/skills/commit/SKILL.md). Apply shared fixes to both copies; only invocation syntax and tool wording intentionally differ. -->
# commit
...
Follow these phases in order. There is no preview or approval gate — once changes are grouped and messages are written, stage and commit straight away.
...
- **Dependency bumps** (`package.json`, `pnpm-lock.yaml`, `requirements.txt`, etc.) → separate, unless directly required by another commit in this batch (then commit the dep first, then the code that uses it).
...
### Phase 3 — Write the commit message(s)

For each commit candidate, write a message in Conventional Commits format with an itemized body.
...
- <bullet explaining a why / change>
- <bullet explaining a why / change>
...
- Write the body as a bulleted list (one `-` per line) — **not** prose paragraphs. One concern or reason per bullet.
- Lead each bullet with the **why** — what problem it solves, what the motivation was. The diff already shows *what*.
- For a non-obvious change, add a bullet on the approach and why alternatives were rejected.
- Add bullets for follow-ups, known limitations, or things deliberately left out.
- Keep each bullet concise; wrap at 72 chars and indent continuation lines to align under the bullet text.
- Don't restate the subject. Don't narrate ("this commit does X") — each bullet says what the change accomplishes.
- Skip the body entirely only for trivial commits where the subject is fully self-explanatory (typo fixes, version bumps, simple renames).
...
- `Co-Authored-By: Claude ...`, `Co-Authored-By: Codex ...`, or any
  Claude/Anthropic/Codex/OpenAI attribution. Commits are authored solely by the
  user.
- "Generated with Claude Code" or "Generated with Codex" footers.
...
### Phase 4 — Stage and commit, one group at a time

There is no preview step — commit each group right away. For each commit candidate:
...
- Sessions expired at 1h with no way to extend, forcing users to
  re-auth mid-task; add a refresh endpoint that rotates the access
  token while keeping the refresh token sliding-window valid for 30 days
- Store refresh tokens hashed with a per-user salt; revoking a user
  invalidates all their refresh tokens via the existing revocation list
...
### Phase 5 — Handle pre-commit hook failures
...
- **Never** add Claude, Anthropic, Codex, or OpenAI co-author/generated-by trailers.
