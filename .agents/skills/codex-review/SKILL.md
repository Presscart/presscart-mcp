---
name: codex-review
description: Use when the user asks for a Codex review, a second-opinion review, invokes $codex-review, or writes the legacy /codex-review phrase — for either CODE (a commit, a phase, or a whole branch) or a DESIGN SPEC / planning doc before implementation. Sends the material to Codex GPT-5.5 for multi-round review, fixes findings, and resubmits until APPROVE or the round cap. Trigger it for code review AND whenever the user wants a spec/plan/design/RFC stress-tested — phrases like "review this spec", "is this plan ready to build", "any gaps before we implement", "validate this design against the codebase", or after brainstorming produces a design doc. Auto-detects design-spec / commit / phase / branch mode and scales the prompt accordingly.
---

<!-- Mirrored skill: a Claude Code twin of this file ships in the kane-claude bundle (.claude/skills/codex-review/SKILL.md). Apply shared fixes to both copies; only invocation syntax and tool wording intentionally differ. -->
# Codex Review
...
In Codex, invoke it as `$codex-review` or through `/skills`. Treat legacy prompt text `/codex-review` as the same request. The mode picks itself.
...
- Notable conventions (if visible in the repo): <e.g. "strict TypeScript", "ESM only", "named exports preferred">
...
```
## Correctness
- Props / inputs typed strictly?
- Conditional rendering covers loading / empty / error states?
- Event handlers cleaned up (listeners, timers, subscriptions)?
- Re-render behavior intentional (memoization where it matters, not everywhere)?
...
## Test quality
- Render test covers the default state?
- Interaction tests for buttons / inputs / toggles?
- a11y assertions where applicable (e.g. via jest-axe or by-role queries)?
```
...
### Auth / session / middleware work

```
## Security
- Session validation in middleware before protected routes?
- No client-side-only auth checks (server must validate)?
- Redirect-after-login parameter sanitized (no open redirect)?
- CSRF on state-changing requests?
- Token storage: httpOnly cookies preferred over localStorage for session tokens?
- No token leakage in logs / monitoring / URLs?

## Correctness
- Guard chain order matches the documented flow?
- Login / register / logout flows handle all states (already-logged-in, invalid token, expired session)?
- Logout clears all client state (caches, in-memory stores)?
```
...
- Read-only sandbox for Codex — the reviewer must never modify the working tree; fixes are applied by the current Codex session. Set `--sandbox read-only` **once** on the Round 1 `exec` call. It is session-scoped and inherited by every `resume`, so never re-pass `--sandbox`/`-C` on resume. **Never pass `--full-auto`** — it is deprecated and silently overrides `--sandbox read-only` with `workspace-write`, granting the reviewer write access; `codex exec` is non-interactive without it.
