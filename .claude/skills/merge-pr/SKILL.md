---
name: merge-pr
description: Use when the user wants to merge a pull request — phrases like "/merge-pr", "merge this PR", "merge it", "land this branch", "squash and merge". Confirms the PR is mergeable, resolves any merge conflict against the base until the branch is clean, then squash-merges and cleans up the branch or worktree. Use this even when the PR shows a conflict — resolving it to a clean, mergeable state is the job, not a reason to stop.
---
